...
public class YangParser implements PsiParser, LightPsiParser {
    ...
    public static boolean VCHAR(PsiBuilder b, int l) {
        ...
        if (!r) r = consumeToken(b, YANG_DOUBLE_FORWARD_SLASH);
    ...
    /* ********************************************************** */
    // augment-arg | (DQUOTE augment-arg (string-splitter augment-arg)* DQUOTE)
    ...
    /* ********************************************************** */
    // BLOCK_COMMENT | (DOUBLE_FORWARD_SLASH (DQUOTE | LEFT_BRACE | RIGHT_BRACE | SEMICOLON | VCHAR | SPACE | TAB)+ )
    ...
    // DOUBLE_FORWARD_SLASH (DQUOTE | LEFT_BRACE | RIGHT_BRACE | SEMICOLON | VCHAR | SPACE | TAB)+
    ...
    // (DQUOTE | LEFT_BRACE | RIGHT_BRACE | SEMICOLON | VCHAR | SPACE | TAB)+
    ...
    /* ********************************************************** */
    // (SINGLE_QUOTE | ESCAPES | VCHAR | SPACE | SEMICOLON | LEFT_BRACE | RIGHT_BRACE | TAB | LINEFEED sep | LINEFEED | CARRIAGE_RETURN)*
    ...
    private static boolean double_quoted_vchar_0(PsiBuilder b, int l) {
        ...
        if (!r) r = consumeToken(b, YANG_RIGHT_BRACE);
        if (!r) r = consumeToken(b, YANG_TAB);
        if (!r) r = double_quoted_vchar_0_8(b, l + 1);
    ...
    // LINEFEED sep
    private static boolean double_quoted_vchar_0_8(PsiBuilder b, int l) {
        if (!recursion_guard_(b, l, "double_quoted_vchar_0_8")) return false;
        boolean r;
        Marker m = enter_section_(b);
        r = consumeToken(b, YANG_LINEFEED);
        r = r && sep(b, l + 1);
    ...
    private static boolean module_stmt_10(PsiBuilder b, int l) {
        ...
        while (true) {
            ...
            if (!empty_element_parsed_guard_(b, "module_stmt_10", c)) break;
        }
        return true;
    }
    ...
    /* ********************************************************** */
    // pattern-keyword sep quoted-string optsep
    //   (SEMICOLON |
    //   LEFT_BRACE stmtsep
    //   // these stmts can appear in any order
    ...
    public static boolean pattern_stmt(PsiBuilder b, int l) {
        ...
        p = r; // pin = 1
        r = r && report_error_(b, sep(b, l + 1));
        r = p && report_error_(b, quoted_string(b, l + 1)) && r;
        r = p && report_error_(b, optsep(b, l + 1)) && r;
        r = p && report_error_(b, pattern_stmt_4(b, l + 1)) && r;
        r = p && stmtsep(b, l + 1) && r;
        exit_section_(b, l, m, r, p, null);
        return r || p;
    }
    ...
    private static boolean pattern_stmt_4_1(PsiBuilder b, int l) {
        ...
        p = r; // pin = 1
        r = r && report_error_(b, stmtsep(b, l + 1));
        r = p && report_error_(b, anyOrder(b, l + 1, YangParser::pattern_stmt_4_1_2_0, YangParser::pattern_stmt_4_1_2_1, YangParser::pattern_stmt_4_1_2_2, YangParser::pattern_stmt_4_1_2_3, YangParser::pattern_stmt_4_1_2_4)) && r;
        r = p && consumeToken(b, YANG_RIGHT_BRACE) && r;
        exit_section_(b, l, m, r, p, null);
        return r || p;
    }
    ...
    /* ********************************************************** */
    // (VCHAR | BACK_SLASH | ESCAPES | SPACE | SEMICOLON | LEFT_BRACE | RIGHT_BRACE | TAB | LINEFEED sep | LINEFEED | CARRIAGE_RETURN)*
    ...
    // LINEFEED sep
    private static boolean quoted_vchar_0_8(PsiBuilder b, int l) {
        if (!recursion_guard_(b, l, "quoted_vchar_0_8")) return false;
        boolean r;
        Marker m = enter_section_(b);
        r = consumeToken(b, YANG_LINEFEED);
        r = r && sep(b, l + 1);
    ...
    private static boolean submodule_stmt_10(PsiBuilder b, int l) {
        ...
        while (true) {
            int c = current_position_(b);
            if (!revision_stmt(b, l + 1)) break;
            if (!empty_element_parsed_guard_(b, "submodule_stmt_10", c)) break;
        }
        return true;
    }
