...
public class YangCompletionContributor extends CompletionContributor {
    ...
    public void fillCompletionVariants(@NotNull CompletionParameters parameters, @NotNull CompletionResultSet result) {
        ...
        if (isAfterKeyword) {
            possibleResults = afterKeyworCompletition(parentType);
    ...
    private List<String> afterKeyworCompletition(String parentType) {
        Class<PsiElement> element = getClassType(contextParent);
        if (element.equals(YangTypedefStmt.class)) {
            return MAP_OF_IDENTIFIER_KEYWORDS.getOrDefault(parentType, null);
        } else
