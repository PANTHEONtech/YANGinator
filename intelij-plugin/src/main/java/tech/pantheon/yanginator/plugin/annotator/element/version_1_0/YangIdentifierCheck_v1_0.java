...
public class YangIdentifierCheck_v1_0 extends AbstractYangStmtCheck {
    @Override
    public boolean isApplicable(@NotNull final PsiElement element) {
        return element instanceof YangIdentifier;
    }

    @Override
    public void performCheck(@NotNull final PsiElement element, @NotNull final AnnotationHolder holder) {
        if (element.getText().length() > 2 && element.getText().toLowerCase().startsWith("xml")) {
            holder.newAnnotation(HighlightSeverity.ERROR, "Identifier can't start with \"xml\" in yang 1.0")
